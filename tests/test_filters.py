...
class TestCompilePatterns:
    """Combined-regex optimization and its fallback paths (_compile_patterns)."""

    def test_simple_patterns_combine_into_one_regex(self):
        from ts4k.core.filter import _compile_patterns

        compiled = _compile_patterns([r"foo", r"bar\d+"])
        assert len(compiled) == 1
        assert compiled[0].search("xx BAR7 yy")
        assert not compiled[0].search("nothing here")

    def test_inline_flag_falls_back_to_individual(self):
        from ts4k.core.filter import _compile_patterns

        compiled = _compile_patterns([r"foo", r"(?m)^bar"])
        assert len(compiled) == 2

    def test_backreference_falls_back_to_individual(self):
        from ts4k.core.filter import _compile_patterns

        compiled = _compile_patterns([r"foo", r"(\w+) \1"])
        assert len(compiled) == 2
        assert compiled[1].search("word word")

    def test_invalid_pattern_dropped_in_fallback(self):
        from ts4k.core.filter import _compile_patterns

        compiled = _compile_patterns([r"[invalid", r"(?m)^ok"])
        assert len(compiled) == 1

    def test_filtering_parity_combined_vs_fallback(self):
        msgs = [
            _msg(subject="Daily Digest for Feb 21"),
            _msg(body="lottery winner announced"),
            _msg(subject="keep me"),
        ]
        combined = apply_filters(msgs, {"skip_patterns": [r"daily\s+digest", r"lottery"]})
        fallback = apply_filters(
            msgs, {"skip_patterns": [r"(?i)daily\s+digest", r"(?i)lottery"]}
        )
        assert [m["subject"] for m in combined] == ["keep me"]
        assert [m["subject"] for m in fallback] == ["keep me"]
