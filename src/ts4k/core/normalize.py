...
_STYLE_WIDTH_0 = re.compile(r"width\s*:\s*[01]px")
_STYLE_HEIGHT_0 = re.compile(r"height\s*:\s*[01]px")
...
_STYLE_DIM_0 = re.compile(r"(width|height)\s*:\s*0", re.IGNORECASE)
...
def _remove_tracking_pixels(soup: BeautifulSoup) -> None:
    ...
    for img in soup.find_all("img"):
        ...
        style = img.get("style", "")
        if style:
            if _STYLE_WIDTH_0.search(style) or \
               _STYLE_HEIGHT_0.search(style) or \
...
def _remove_hidden_elements(soup: BeautifulSoup) -> None:
    ...
    for el in soup.find_all(style=_STYLE_DIM_0):
